...
| Version pin | `init` writes `.github/workflows/accord.yml` with `npx --yes @<scope>/accord@<version>` using the CLI's own `package.json` version, and writes the same version into `config.yml`. A mismatch is a hard refusal on every repository-reading command (`lint`, `gate ready`, `gate done`, `status`, `new ticket`): exit 2, with a message naming the pinned version, the running version, and the fix command. Comparison is exact string equality — no semver range, no tolerance. There is no bypass flag and no environment variable; only `--version` and `--help` are exempt, because `--version` is what a person types to diagnose the mismatch. | D-94, D-95. PITFALLS.md §11, whose "(or warns, per profile)" parenthetical is superseded: a warning is what an agent ignores, and D-88 makes `maintain` the looser profile everywhere else, so a profile-dependent pin inverted the semantics. The CLI itself never spawns npm/npx (§12). |
